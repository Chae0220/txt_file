...
class StrategySma0(Strategy):
    ...
    def __update_process(self, info):
        try:
            ...
            if sma_mid < current_price and sma_short > sma_mid > sma_long and self.current_process != "buy":    #mid current
                ...
                if current_idx > self.LONG:
                    ...
                    if std_ratio > self.STD_RATIO:
                        ...
                        self.logger.debug(f"[SMA] SKIP BUY !!! === Strand deviation: {std_ratio:.6f}")
                    elif prev_sell_idx + self.CALM_DOWN > current_idx:
                        self.cross_info[1] = {"price": 0, "index": current_idx}
                        self.logger.debug(f"[SMA] SKIP BUY !!! === Too early, before: {current_idx - prev_sell_idx}")

                # self.__add_drawing_spot(info["date_time"], current_price)

            elif sma_mid > current_price and sma_short < sma_mid < sma_long and self.current_process != "sell":          #mid current
